...
export default function Login() {
  ...
  // OAuth redirect lands back with an active session
  if (user) {
    window.location.href = '/search'
    return null
  }
  ...
  return (
    <Shell>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <OAuthButton onClick={() => handleOAuth('github')} icon={<GithubIcon />} label="Sign in with Github" />
        <OAuthButton onClick={() => handleOAuth('google')} icon={<GoogleIcon />} label="Sign in with Google" />
        <OAuthButton onClick={() => handleOAuth('azure')} icon={<AzureIcon />} label="Sign in with Azure" />
      </div>
